...
complement the analysis done by Metabolon. In this vignette we demonstrate how to use
MetabolomicsPipeline package for:
...
of these datasets together as a [SummarizedExperiment](https://bioconductor.org/packages/release/bioc/vignettes/SummarizedExperiment/inst/doc/SummarizedExperiment.html) using loadMetabolon(). However, for this vignette we are using the
demo data provided with the package. This data is accessed using MetabolomicsPipeline::demoDat. In the following chunk we:
...
a sample metadata variable to label the points in the plot. The metabolite_pca function
has three arguments:
...
amount of the metabolite variance with treatment included, and the treatment model is testing if
...
Assay	
Name of the assay to be used for the pairwise analysis (default='normalized')

mets	
Chemical ID for the metabolites of interest. If NULL then the pairwise analysis is completed for all metabololites.

form:	
This is a character string the resembles the right hand side of a simple linear regression model in R. For example form = "Group1 + Group2".

data:	
SummarizedExperiment with Metabolon experiment data.

strat_var:	
A variable in the analysis data to stratify the model by. If this is specified, a list of results will be returned.
