...
met_est_heatmap <- function(results_data, data, interactive = FALSE, ...) {
    ...
    dat <- rowData(data) %>%
        ...
        dplyr::select(
            CHEM_ID, SUB_PATHWAY, CHEMICAL_NAME,
            all_of(names(results_data)[grepl("ESTS", names(results_data))])
        ) %>%
        reshape2::melt(
    ...
    if (interactive == FALSE) {
        ...
        matr <- dat %>%
            reshape2::dcast(CHEMICAL_NAME ~ Contrast, value.var = "logFoldChange")
