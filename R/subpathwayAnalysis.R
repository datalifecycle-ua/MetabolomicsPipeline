...
#'  treatment and the block variable. The parallel model tests the treatment and
#'  the block factors using type II ANOVA, and the single model tests if the treatment
#'  explains a significant amount of the metabolite variance. Then, we 
#'  use the Combined Fisher probability to test each model at the subpathway 
#'  level.
#'
...
subpathwayAnalysis <- function(data, treat_var, block_var = NULL,
    ...
    if (is.null(strat_var)) {
        if (!is.null(block_var)) {
            ...
            path_data <- data.frame(
                ...
                parallel_block_pval = NA,
                parallel_block_fisher = NA,
                parallel_treatment_pval = NA,
                parallel_treatment_fisher = NA,
                single_pval = NA,
                single_fisher = NA,
                model = NA
            ) %>%
            ...
            for (pathway in unique(path_data$sub_pathway)) {
                ...
                for (var in var_names) {
                    ...
                    tryCatch(
                        ...
                        error = function(e){
                          warning(e, " for CHEM ID ", var, ". Metabolite is
                                    excluded.")
                          path_data$interaction_pval[
                            which(path_data$CHEM_ID == var)] <- NA
                        }
                    ...
                    tryCatch(
                        {
                            path_data$parallel_block_pval[which(
                                path_data$CHEM_ID == var
                            )] <- Anova(par_mod)[block_var, 4]
                            
                            path_data$parallel_treatment_pval[which(
                              path_data$CHEM_ID == var
                            )] <- Anova(par_mod, type = 2)[treat_var, 4]
                        },
                        warning = function(w) {
                            warning(w, " for CHEM ID ", var, ". Metabolite is
                                    excluded.")
                          path_data$parallel_block_pval[
                            which(path_data$CHEM_ID == var)
                          ] <- NA
                          
                          path_data$parallel_treatment_pval[which(
                            path_data$CHEM_ID==var)] <- NA
                        },
                        error = function(e){
                              warning(e, " for CHEM ID ", var, ". Metabolite is
                                    excluded.")
                ...
                if (length(var_names) == 1) {
                    ...
                    path_data$parallel_block_fisher[
                        which(path_data$CHEM_ID == var)
                    ] <- Anova(par_mod, type = 2)[block_var, 4]
                    
                    path_data$parallel_treatment_fisher[
                      which(path_data$CHEM_ID == var)
                    ] <- Anova(par_mod, type = 2)[treat_var, 4]
                    ...
                    path_data$model[which(path_data$sub_pathway == pathway)] <-
                        case_when(
                            anova(int_mod)[
                                ...
                                "Interaction",
                            Anova(par_mod, type = 2)[block_var, 4] < 0.05 ~ "Parallel",
                    ...
                    # Calculate parallel block fisher
                    parallel_block_fisher <- -2 * sum(log(path_data$parallel_block_pval[
                        which(path_data$sub_pathway == pathway)
                    ]), na.rm = TRUE)
                    
                    

                    parallel_block_p <- round(
                        pchisq(parallel_block_fisher, 2 * length(var_names),
                            lower.tail = FALSE
                        ), 2
                    )
                    ...
                    # Calculate parallel treatment fisher
                    parallel_treatment_fisher <- -2 * sum(log(path_data$parallel_treatment_pval[
                      which(path_data$sub_pathway == pathway)
                    ]), na.rm = TRUE)
                    
                    parallel_treatment_p <- round(
                      pchisq(parallel_treatment_fisher, 2 * length(var_names),
                             lower.tail = FALSE
                      ), 2
                    )
                    ...
                    path_data$parallel_treatment_fisher[
                      which(path_data$sub_pathway == pathway)
                    ] <- parallel_treatment_p
                    
                    # Calculate single fisher    
                    ...
                    path_data$model[which(path_data$sub_pathway == pathway)] <-
                        case_when(
                            ...
                            parallel_block_p < 0.05 ~ "Parallel",
                ...
                for (pathway in unique(path_data$sub_pathway)) {
                    ...
                    for (var in var_names) {
                        ...
                        tryCatch(
                            ...
                            warning = function(w) {
                              warning(w, " for CHEM ID ", var, " in the ",
                                      strats[i], " observations. Metabolite is
                                          excluded.")
                                path_data$interaction_pval[
                                  which(path_data$CHEM_ID == var)
                                ] <- NA
                            ...
                            error = function(e){
                              warning(e, " for CHEM ID ", var, " in the ",
                                      strats[i], " observations. Metabolite is
                                          excluded.")
                              path_data$interaction_pval[
                                which(path_data$CHEM_ID == var)
                        ...
                        tryCatch(
                          ...
                          warning = function(w) {
                            warning(w, " for CHEM ID ", var, " in the ",
                                    strats[i], " observations. Metabolite is excluded.")
